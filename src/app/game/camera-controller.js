...
class CameraController {
	...
	moveCamera(vector) {
		...
		if (vector.x < 0 && 500 - map.width * constants.tileWidth < this._camera.offsetX) {
			if (this._camera.offsetX + vector.x < 500 - map.width * constants.tileWidth) {
				this._camera.offsetX = 500 - map.width * constants.tileWidth;
			} else {
				this._camera.offsetX += vector.x;
			}
		}

		if (vector.y < 0 && 500 - map.height * constants.tileWidth < this._camera.offsetY) {
			if (this._camera.offsetY + vector.y < 500 - map.height * constants.tileWidth) {
				this._camera.offsetY = 500 - map.height * constants.tileWidth;
			} else {
				this._camera.offsetY += vector.y;
			}
		}
